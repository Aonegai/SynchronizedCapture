...
public class Socket: SocketReader, SocketWriter {
	...
	public enum Address {
        ...
        public func ipAddressString() -> String {
                switch self {
                case .ipv4(let addr):
                    var addr = addr
                    var ipAddressStringBuffer = [CChar](repeating: 0, count: Int(INET_ADDRSTRLEN))
                    let ipAddressPointer = withUnsafePointer(to: &addr.sin_addr) {
                        return inet_ntop(AF_INET, $0, &ipAddressStringBuffer, socklen_t(INET_ADDRSTRLEN))
                    }
                    return ipAddressPointer.map { String(cString: $0) }!
                    
                case .ipv6(let addr):
                    var addr = addr
                    var ipAddressStringBuffer = [CChar](repeating: 0, count: Int(INET6_ADDRSTRLEN))
                    let ipAddressPointer = withUnsafePointer(to: &addr.sin6_addr) {
                        return inet_ntop(AF_INET6, $0, &ipAddressStringBuffer, socklen_t(INET6_ADDRSTRLEN))
                    }
                    return ipAddressPointer.map { String(cString: $0) }!
                    
                case .unix(_):
                    // UNIX域地址不直接对应到IP地址，根据用途返回合适的表示
                    return "UNIX Domain"
                }
            }
	}
	...
	///
	public class func createAddress(for host: String, on port: Int) -> Address? {
    ...
    public func readDatagram() throws -> (data: [UInt8], bytesRead: Int, address: Address?) {
        ...
        var data = [UInt8]()
        if count > 0 {
            // - If data is available, prepare the data array from the storage
            data = Array(UnsafeBufferPointer(start: self.readStorage.bytes.assumingMemoryBound(to: UInt8.self), count: self.readStorage.length))
    ...
    @discardableResult public func write(from data: [UInt8], to address: Address) throws -> Int {

        // Send the bytes...
        return try write(from: data, bufSize: data.count)
    }
